...
func (r *Resolver) goLookupIP(ctx context.Context, network, host string) (addrs []IPAddr, err error) {
	addrs, _, err = r.goLookupIPCNAME(ctx, network, host, getSystemDNSConfig())
	return
}

func (r *Resolver) goLookupIPCNAME(ctx context.Context, network, name string, conf *dnsConfig) (addrs []IPAddr, cname dnsmessage.Name, err error) {
	if !isDomainName(name) {
